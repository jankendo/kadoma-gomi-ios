...
- 添付画像の固有キャラクター、アプリ名、独自アイコンは使用していない。
- シンプルUI化に合わせ、追加画像ではなくSF Symbols中心の既存アイコンを整理して使用する方針にした。
- `AppIcon`、`WasteSymbol`、カテゴリ色を使い、ホーム、検索、ごみ分別、カレンダーで統一した意味表現を維持する。
- 過度なイラストや装飾画像は追加していない。門真市公式情報の信頼性と日常確認の読みやすさを優先するため。
