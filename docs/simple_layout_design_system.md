...
- シンプルUI方針は維持し、追加操作は小さなリンク、1行ステータス、開発者用画面に閉じ込めた。
- ホームとカレンダーでは大きなカードを増やさず、既存のリスト/グリッド内に操作を追加した。
- ごみ分別カードは `188pt` 目安の統一高さとし、アイコン・タイトル・要約・品目数の位置を揃えた。
- 開発者向け情報は通常設定のカード群から分離し、一般ユーザーの情報密度を上げない。
